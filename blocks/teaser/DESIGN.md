...
| 方針 | 内容 |
|------|------|
| JS 不使用 | ブロック専用の `teaser.js` は持たない。DOM 整形は Franklin 共通処理に委ねる |
...
| `imageAlt` | title | ✓ | PC 用代替テキスト。`image` と Field Collapse で `<picture>` に合成 |
| `imageMobile` | reference | — | SP 用画像アセット（任意）。未設定時は PC 画像を全画面幅で使用 |
| `imageMobileAlt` | title | — | SP 用代替テキスト。`imageMobile` と Field Collapse で合成 |
